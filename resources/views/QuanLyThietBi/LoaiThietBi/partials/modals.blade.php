...
<div id="addModal" class="fixed inset-0 bg-gray-600 bg-opacity-50 hidden overflow-y-auto h-full w-full z-50">
    <div class="relative top-20 mx-auto p-5 border w-96 shadow-lg rounded-md bg-white">
        ...
        <form action="{{ route('loaithietbi.create') }}" method="POST">
            @csrf
            <div class="mb-4">
                <label for="ten_kv" class="block text-sm font-medium text-gray-700">Tên loại thiết bị</label>
                <input type="text" name="ten_ltb" id="ten_ltb" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500" required>
            </div>
            <div class="mb-4">
...
<div id="editModal" class="fixed inset-0 bg-gray-600 bg-opacity-50 hidden overflow-y-auto h-full w-full z-50">
    <div class="relative top-20 mx-auto p-5 border w-96 shadow-lg rounded-md bg-white">
        <div class="flex justify-between items-center mb-4">
            <h3 class="text-lg font-medium text-gray-900">Chỉnh sửa loại thiết bị</h3>
        ...
        <form id="editForm" method="POST">
            ...
            @method('PUT')
            <div class="mb-4">
                <label for="edit_ten_kv" class="block text-sm font-medium text-gray-700">Tên loại thiết bị</label>
                <input type="text" name="edit_ten_ltb" id="edit_ten_ltb" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500" required>
            </div>
            <div class="mb-4">
                <label for="edit_mota" class="block text-sm font-medium text-gray-700">Mô tả</label>
                <textarea name="edit_mota" id="edit_mota" rows="3" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500"></textarea>
            </div>
            <div class="flex justify-end space-x-3">
...
<script>
    function openEditModal(id, ten, mota) {
        document.getElementById('editModal').classList.remove('hidden');
        document.getElementById('edit_ten_ltb').value = ten;
        document.getElementById('edit_mota').value = mota;
        document.getElementById('editForm').action = `/thiet-biloai-thiet-bi/edit/${id}`;
    }

    function openDeleteModal(id) {
        document.getElementById('deleteForm').action = `/thiet-bi/loai-thiet-bi/delete/${id}`;
