...
<div class="wbody">
    <h3 class="wtitle">    Quản lý loại thiết bị</h3>
    ...
    <div class="wtable-container">
        <table class="wtable" id="tableLoaiThietBi">
            <thead>
                <tr>
            ...
            <tbody>
                @foreach ($loaiThietBi as $key => $item)
                <tr>
                    <td style="width: 100px">{{ $key + 1 }}</td>
                    <td style="width: 100px">{{ $item->id_ltb }}</td>
                    <td>{{ $item->ten_ltb }}</td>
                    <td>{{ $item->mota}}</td>
                    <td style="width: 100px">
                        <div class="wdiv_group">
                            <button class="wbutton update" onclick="openEditModal('{{ $item->id_ltb }}', '{{ $item->ten_ltb }}', '{{ $item->mota }}')">
                                <i class="fas fa-edit"></i>    
                            </button>
                            <button class="wbutton delete" onclick="openDeleteModal('{{ $item->id_ltb }}')">
    ...
    <div class="wpagination">
        ...
        @else
            <a href="{{ $loaiThietBi->url(1) }}" class="wpagination-item">
                <i class="fas fa-angle-double-left"></i>
            </a>
            <a href="{{ $loaiThietBi->previousPageUrl() }}" class="wpagination-item">
                <i class="fas fa-chevron-left"></i>
            </a>
        @endif

        @foreach ($loaiThietBi->getUrlRange(1, $loaiThietBi->lastPage()) as $page => $url)
            <a href="{{ $url }}" 
            class="wpagination-item {{ $page == $loaiThietBi->currentPage() ? 'active' : '' }}">
                {{ $page }}
            </a>
        @endforeach

        @if ($loaiThietBi->hasMorePages())
            <a href="{{ $loaiThietBi->nextPageUrl() }}" class="wpagination-item">
                <i class="fas fa-chevron-right"></i>
            </a>
            <a href="{{ $loaiThietBi->url($loaiThietBi->lastPage()) }}" class="wpagination-item">
...
function searchTable() {
    ...
    var table = document.getElementById("tableLoaiThietBi");
